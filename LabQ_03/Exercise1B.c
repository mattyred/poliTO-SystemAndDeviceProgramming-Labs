...
/*
Non-blocking I/O: the writing operation on the pipeline is non blocking. It means
that we don't have to wait until the write operation is terminated to go on
*/
void set_fcntl(int fd, int flags){
    int val;
    if((val = fcntl(fd, F_GETFL, 0)) < 0){
        printf("Error in fcntl - 1");
        exit(-1);
    }
    val |= flags;
    if(fcntl(fd, F_SETFL, val) < 0){
        printf("Erro in fcntl - 2");
        exit(-1);
    }
}
int main(int argc, char *argv[])
{
    ...
    if ((child[0] = fork()) == 0)
    {
        ...
        while (i > 0)
        {
            ...
            str[length] = '\0';
            /* Writing section */
            set_fcntl(fd1[1], O_NONBLOCK); // set fd[0] in non-blocking mode
    ...
    else
    {
        ...
        if ((child[1] = fork()) == 0)
        {
            ...
            while (i > 0)
            {
                ...
                /* Writing section */
                set_fcntl(fd2[1], O_NONBLOCK); // set fd[1] in non-blocking mode
